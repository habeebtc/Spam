...
namespace yaf.pages
{
  public class ForumPageArgs : EventArgs
  {
	...
	public class ForumPage : System.Web.UI.UserControl
	{
		...
		private bool m_renderHead = true;
    ...
    public event EventHandler<ForumPageArgs> PageTitleSet;
      ...
      if ( PageTitleSet != null ) PageTitleSet( this, new ForumPageArgs( _forumPageTitle ) );
		...
		private void SendMailThread()
		{
			try
			{
				System.Net.Mail.SmtpClient smtpSend = new System.Net.Mail.SmtpClient( BoardSettings.SmtpServer );
				// short timeout...
				smtpSend.Timeout = 10000;

				if ( BoardSettings.SmtpUserName != null && BoardSettings.SmtpUserPass != null )
				{
					smtpSend.Credentials = new System.Net.NetworkCredential( BoardSettings.SmtpUserName, BoardSettings.SmtpUserPass );
				}
					...
					for ( int i = 0; i < dt.Rows.Count; i++ )
					{
						...
						if ( dt.Rows [i] ["ToUser"].ToString().Trim() != String.Empty )
						{
							try
							{
								...
								{
									emailMessage.To.Add( dt.Rows [i] ["ToUser"].ToString() );
									emailMessage.From = new System.Net.Mail.MailAddress( BoardSettings.ForumEmail );
									emailMessage.Subject = dt.Rows [i] ["Subject"].ToString();
									emailMessage.Body = dt.Rows [i] ["Body"].ToString();

									if ( !System.Text.RegularExpressions.Regex.IsMatch( emailMessage.Body, @"^([0-9a-z!@#\$\%\^&\*\(\)\-=_\+])", System.Text.RegularExpressions.RegexOptions.IgnoreCase ) ||
											!System.Text.RegularExpressions.Regex.IsMatch( emailMessage.Subject, @"^([0-9a-z!@#\$\%\^&\*\(\)\-=_\+])", System.Text.RegularExpressions.RegexOptions.IgnoreCase ) )
									{
							...
							{
								// we still want to continue and delete								
							}
							catch
							{
								throw;
    ...
    public yaf.Forum ForumControl
    {
      ...
      set
      {
		...
		protected void SetupHeadControl( ref System.Web.UI.HtmlControls.HtmlHead head )
		{
			...
			head.Controls.Add( MakeStyleSheetControl( String.Format( "{0}forum.css", Data.ForumRoot ) ) );
			head.Controls.Add( MakeStyleSheetControl( ThemeFile( "theme.css" ) ) );

			if ( m_strRefreshURL != null && m_nRefreshTime >= 0 )
			{
				HtmlMeta refresh = new HtmlMeta();
				refresh.HttpEquiv = "Refresh";
				refresh.Content = String.Format( "{1};url={0}", m_strRefreshURL, m_nRefreshTime );
      ...
      // BEGIN HEADER
		...
		public int PageCategoryID
		{
			...
			{
				if ( ForumControl.CategoryID != 0 )
					return ForumControl.CategoryID;
