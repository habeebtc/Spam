...
namespace yaf.pages
{
	...
	public partial class posts : ForumPage
	{
		...
		protected void Page_Load( object sender, System.EventArgs e )
		{
			try
			{
				...
				if ( !IsPostBack )
				{
					...
					{
						PageLinksTop.AddLink( BoardSettings.Name, Forum.GetLink( Pages.forum ) );
						PageLinksTop.AddLink( PageCategoryName, Forum.GetLink( Pages.forum, "c={0}", PageCategoryID ) );
						PageLinksBottom.AddLink( BoardSettings.Name, Forum.GetLink( Pages.forum ) );
						PageLinksBottom.AddLink( PageCategoryName, Forum.GetLink( Pages.forum, "c={0}", PageCategoryID ) );
					}
					...
					PageLinksTop.AddForumLinks( PageForumID );
					PageLinksTop.AddLink( Utils.BadWordReplace( PageTopicName ), "" );
					PageLinksBottom.AddForumLinks( PageForumID );
					PageLinksBottom.AddLink( Utils.BadWordReplace( PageTopicName ), "" );
			...
			{
				Response.Write( "Got exception = " + x.Message + " source = " + x.Source + " stacktrace = " + x.StackTrace );
			}
		}
		...
		{
			return HtmlEncode(Utils.BadWordReplace(dtPoll.Rows [0] ["Question"].ToString()));
		}
