...
namespace yaf.controls
{
	public partial class EditUsersProfile : BaseUserControl
	{
		...
		private void BindData()
		{
			...
			Location.Text = BBCode.EncodeHTML(row["Location"].ToString());
			HomePage.Text = BBCode.EncodeHTML(row["HomePage"].ToString());
			TimeZones.Items.FindByValue( row ["TimeZone"].ToString() ).Selected = true;			
			Email.Text = row ["Email"].ToString();
			Realname.Text = BBCode.EncodeHTML(row["RealName"].ToString());
			Occupation.Text = BBCode.EncodeHTML(row["Occupation"].ToString());
			Interests.Text = BBCode.EncodeHTML(row["Interests"].ToString());
