...
namespace yaf.pages
{
	...
	public partial class postmessage : ForumPage
	{
		...
		protected void Page_Load( object sender, System.EventArgs e )
		{
			...
			if ( !IsPostBack )
			{
				bool isNewTopic = ( Request.QueryString ["t"] == null ) && ( Request.QueryString ["q"] == null ) && ( Request.QueryString ["m"] == null );
				...
				NewTopicOptionsRow.Visible = isNewTopic && !this.IsGuest;
				if ( isNewTopic && this.ForumUploadAccess )
				{
		...
		protected void PostReply_Click( object sender, System.EventArgs e )
		{
			...
			if ( bApproved )
			{
				...
				if ( isNewTopic && this.ForumUploadAccess && TopicAttach.Checked )
				{
					// redirect to the attachment page...
					Forum.Redirect( Pages.attachments, "m={0}", nMessageID );
				}
				else
				{
					// regular redirect...
